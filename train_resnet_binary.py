...
def train_resnet_binary(num_epochs: int = 30, batch_size: int = 32, lr: float = 1e-4) -> None:
    ...
    model = build_resnet18_binary(num_classes=2).to(device)
    ...
    print(f"Model ResNet18 biner tersimpan di: {MODEL_PATH.resolve()}")
    print(f"Label biner tersimpan di: {LABELS_PATH.resolve()}")
    print(f"Riwayat training tersimpan di: {HISTORY_PATH.resolve()}")


if __name__ == "__main__":
    train_resnet_binary(num_epochs=30, batch_size=32, lr=1e-4)
